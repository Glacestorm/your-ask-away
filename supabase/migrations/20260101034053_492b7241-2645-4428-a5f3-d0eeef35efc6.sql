...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'erp_suppliers' AND policyname = 'erp_suppliers_policy') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'erp_warehouses' AND policyname = 'erp_warehouses_policy') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'erp_warehouse_locations' AND policyname = 'erp_warehouse_locations_policy') THEN
...
-- Triggers updated_at (usando función existente public.update_updated_at_column)
DROP TRIGGER IF EXISTS tr_suppliers_updated ON erp_suppliers;
CREATE TRIGGER tr_suppliers_updated BEFORE UPDATE ON erp_suppliers FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS tr_warehouses_updated ON erp_warehouses;
CREATE TRIGGER tr_warehouses_updated BEFORE UPDATE ON erp_warehouses FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();