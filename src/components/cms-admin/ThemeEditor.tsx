...
export function ThemeEditor() {
  ...
  const loadThemes = async () => {
    try {
      const { data, error } = await (supabase as any).from('cms_themes').select('*').order('created_at', { ascending: false });
  ...
  const saveTheme = async () => {
    ...
    try {
      await (supabase as any).from('cms_themes').update({ color_palette: editedColors }).eq('id', selectedTheme.id);
  ...
  const activateTheme = async (theme: Theme) => {
    try {
      await (supabase as any).from('cms_themes').update({ is_active: false }).neq('id', theme.id);
      await (supabase as any).from('cms_themes').update({ is_active: true }).eq('id', theme.id);
  ...
  const createFromPreset = async (preset: typeof presets[0]) => {
    try {
      await (supabase as any).from('cms_themes').insert({
  ...
  const deleteTheme = async (id: string) => {
    try {
      await (supabase as any).from('cms_themes').delete().eq('id', id);
