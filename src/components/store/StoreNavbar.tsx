      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/store" className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent ml-1" style={{ fontFamily: 'Crimson Pro, serif' }}>
              Store
            </span>
          </Link>

          {/* Desktop Navigation - 3D Pill Buttons */}
          ...
          <div className="flex items-center gap-4">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.9 }}
              className="relative p-3 rounded-xl bg-gradient-to-b from-slate-700 to-slate-800 shadow-[0_4px_12px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)] hover:from-slate-600 hover:to-slate-700 transition-all duration-300"
            >
              <ShoppingCart className="w-6 h-6 text-slate-200" />
              {itemCount > 0 && (
                <>
                  {/* Pulse rings */}
                  <span className="absolute inset-0 rounded-xl animate-ping bg-emerald-500/30" />
                  <span className="absolute inset-0 rounded-xl animate-pulse bg-emerald-500/20" style={{ animationDelay: '0.5s' }} />
                  {/* Badge */}
                  <Badge 
                    className="absolute -top-2 -right-2 w-6 h-6 p-0 flex items-center justify-center bg-gradient-to-r from-emerald-400 to-cyan-400 text-white text-xs font-bold shadow-[0_2px_8px_rgba(16,185,129,0.5)] border-2 border-slate-950"
                  >
                    {itemCount}
                  </Badge>
                </>
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.05, y: -2 }}
              whileTap={{ scale: 0.95, y: 1 }}
              className="hidden md:flex items-center gap-2 px-7 py-3 rounded-xl font-bold text-sm tracking-wide bg-gradient-to-b from-emerald-400 via-emerald-500 to-emerald-700 text-white shadow-[0_6px_24px_rgba(16,185,129,0.5),inset_0_1px_0_rgba(255,255,255,0.4),inset_0_-3px_6px_rgba(0,0,0,0.2)] hover:from-emerald-300 hover:via-emerald-400 hover:to-emerald-600 hover:shadow-[0_8px_32px_rgba(16,185,129,0.6)] transition-all duration-300"
              style={{ fontFamily: 'Crimson Pro, serif' }}
            >
