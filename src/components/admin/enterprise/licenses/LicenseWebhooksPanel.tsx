...
import { useLicenseWebhooks, LicenseWebhook } from '@/hooks/admin/enterprise';
import { formatDistanceToNow } from 'date-fns';
import { es } from 'date-fns/locale';
import { toast } from 'sonner';

const EVENT_TYPES = [
  { value: 'license.created', label: 'Licencia Creada', description: 'Cuando se crea una nueva licencia' },
  { value: 'license.activated', label: 'Licencia Activada', description: 'Cuando una licencia es activada' },
  { value: 'license.expired', label: 'Licencia Expirada', description: 'Cuando una licencia expira' },
  { value: 'license.renewed', label: 'Licencia Renovada', description: 'Cuando una licencia se renueva' },
  { value: 'license.suspended', label: 'Licencia Suspendida', description: 'Cuando una licencia es suspendida' },
  { value: 'device.activated', label: 'Dispositivo Activado', description: 'Cuando se activa un dispositivo' },
  { value: 'device.deactivated', label: 'Dispositivo Desactivado', description: 'Cuando se desactiva un dispositivo' },
  { value: 'anomaly.detected', label: 'Anomalía Detectada', description: 'Cuando se detecta uso sospechoso' },
];

export function LicenseWebhooksPanel() {
  const [activeTab, setActiveTab] = useState('webhooks');
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [selectedWebhook, setSelectedWebhook] = useState<LicenseWebhook | null>(null);
  ...
  const handleToggleWebhook = async (webhook: LicenseWebhook) => {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => { fetchWebhooks(); fetchWebhookLogs(); }}>
